...
from geeflow.composite.builder import build_composites, sensors_for
...
from geeflow.features.seasonal import compute_seasonal_features
from geeflow.features.texture import add_texture
from geeflow.sensors.harmonize import get_collection
from geeflow.session import init_ee
from lulc_engine.aoi import resolve_aoi
...
class Pipeline:
    ...
    def build_features(self, year: int):
        ...
        stack = compute_seasonal_features(with_indices, computed, cfg.seasons, cfg.features.seasonal)
    ...
    def classify(self, year: int, export: bool = True) -> dict:
        ...
        if export:
            task = export_image(
                classified,
                region=self.aoi,
                description=f"{cfg.export.prefix}_{year}_{best_name}",
                target=cfg.export.target,
                folder=cfg.export.folder,
                asset_root=cfg.export.asset_root,
                scale=cfg.export.scale_m or cfg.scale_for_year(year),
                crs=cfg.export.crs,
                max_pixels=cfg.export.max_pixels,
                to_uint8=True,
                log=log,
            )

...
Fallback ladder (per season):
  ...
  2. if fewer than composite.min_scenes: merge the backup sensor (when one exists)
...
from lulc_engine.config.schema import LulcConfig, Season, SensorEra
from lulc_engine.sensors.harmonize import get_collection, harmonized_bands_for
from lulc_engine.sensors.registry import get_sensors_for_year
...
def build_season_collection(
    ...
    primary = sensors[0]
    min_scenes = cfg.composite.min_scenes
...
def build_composites(year: int, aoi, cfg: LulcConfig, log=print):
    ...
    log(f"\nBuilding {len(cfg.seasons)}-season composites for {year} "
        f"(era={era.name}, sensor={primary}, scale={era.scale_m}m)")

    composites = {}
    for season in cfg.seasons:
        col, _, count = build_season_collection(year, aoi, season, era, cfg, log=log)
        if count == 0:
            raise RuntimeError(
                f"No scenes found for {year} season {season.name!r} even after fallbacks. "
                f"Check `lulc check` output and consider widening the season or search window."
            )
        reduced = col.median() if cfg.composite.reducer == "median" else col.mean()
